...
function useInView() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setInView(entry.isIntersecting) // ← quita el "if" para que también se ponga en false
      },
      { threshold: 0.2 }
    )
...
function MainPage() {
  ...
  return (
    <div>
      ...
      {/* ── About section ── */}
      <section className="flex items-center gap-16 px-16 py-24 bg-white">
 
        {/* Video — slides in from left */}
        <div
          ref={videoRef}
          className="relative flex-shrink-0 w-[480px] transition-all duration-700 ease-out"
          style={{
        ...
        >
          {/* Sombras */}
          {/* Sombras que sobresalen por arriba */}
        <div className="absolute bg-[#9B0032] opacity-30" style={{ width: '110%', height: '110%', top: '-15%', left: '-5%', clipPath: 'polygon(8% 0%, 95% 5%, 88% 60%, 3% 55%)', transform: 'translate(-10px, -15px)', filter: 'blur(12px)' }} />
        <div className="absolute bg-[#003e7e] opacity-20" style={{ width: '105%', height: '100%', top: '-10%', left: '0%', clipPath: 'polygon(15% 0%, 100% 8%, 92% 50%, 0% 45%)', transform: 'translate(15px, -20px)', filter: 'blur(16px)' }} />
        <div className="absolute bg-[#9B0032] opacity-40" style={{ width: '120%', height: '90%', top: '-10%', left: '-10%', clipPath: 'polygon(5% 5%, 85% 0%, 95% 55%, 10% 60%)', transform: 'translate(-20px, -10px)', filter: 'blur(10px)' }} />
        <div className="absolute inset-0 opacity-50 bg-[#9B0032]" style={{ clipPath: 'polygon(0% 10%, 90% 0%, 100% 85%, 15% 100%)', transform: 'translate(25px, 35px)', filter: 'blur(8px)' }} />
        <div className="absolute inset-0 opacity-50 bg-[#9B0032]" style={{ height: '150%', clipPath: 'polygon(20% 40%, 90% 0%, 100% 85%, 15% 100%)', transform: 'translate(-105px, -105px)', filter: 'blur(8px)' }} />
        <div className="absolute inset-0 opacity-30 bg-[#003e7e]" style={{ clipPath: 'polygon(5% 0%, 100% 8%, 95% 100%, 0% 90%)', transform: 'translate(-50px, 30px)', filter: 'blur(10px)' }} />
        <div className="absolute inset-0 opacity-20 bg-[#9B0032]" style={{ clipPath: 'polygon(10% 5%, 95% 0%, 100% 90%, 5% 100%)', transform: 'translate(25px, -10px)', filter: 'blur(14px)' }} />
        ...
        >
          <h2 className="text-4xl font-bold text-[#770046]">Play. Learn. Protect.</h2>
          ...
          <p className="text-lg text-gray-700 leading-relaxed font-medium">
            The game combines {' '} <Highlight inView={textInView} delay={700}>RPG-style storytelling</Highlight> with fast-paced {' '} <Highlight inView={textInView} delay={1000}>arcade minigames</Highlight> inspired by real industrial 
            environments and technologies. Players begin in a familiar office setting before being transported into 
            a fictional world where they must overcome cybersecurity challenges across different industries. Each 
            minigame introduces concepts related to industrial security, vulnerabilities, and protection strategies 
            in a fun, accessible, and engaging way.
          </p>
          <p className="text-lg text-gray-700 leading-relaxed font-medium">
